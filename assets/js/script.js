...
.then(data => {
    ...
    if (data.collection?.length) {
            ...
            return (`
                <div class="row justify-content-md-center">
                    <div class="col col-6 time-slot text-center m-2">
                        <a href="#" onclick="Calendly.initPopupWidget({url: '${e.scheduling_url}'});return false">
        ...
        // Wrap the selected date without the <b> tag
        selected_date.innerHTML = `
            <div style="border: 2px solid var(--primary-color); padding: 10px; border-radius: 5px; background-color: #f1f1f1; display: inline-block; margin-top: 20px;">
                ${moment(date, 'YYYY-MM-DD').format('ddd, MMM DD')}
            </div>
        `;
        slot_data.classList.remove('d-none');
        slot_data.innerHTML = slots_list;
    } else {
        // No slots message with similar styling
        selected_date.innerHTML = `
            <div style="border: 2px solid var(--primary-color); padding: 10px; border-radius: 5px; background-color: #f1f1f1; display: inline-block; margin-top: 20px;">
                No Slots found for selected date. Please choose another date.
            </div>
        `;
    }
});
